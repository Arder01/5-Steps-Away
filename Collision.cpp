...
bool Collision::CheckCollision(SDL_Rect a) {
	for (auto i : v) {
		if (SDL_HasIntersection(&a, &i.first)) 
		{
			SetFlag(i.second);
			return true;
		}
...
bool Collision::Parse(std::string id,std:: string source) {
	...
	for (TiXmlElement* group = root->FirstChildElement(); group; group = group->NextSiblingElement()) {
		...
		if (group->QueryStringAttribute("flag", &flag) != TIXML_SUCCESS) {
			std::cout << "Error parsing 'flag' attribute in XML for group." << std::endl;
		}
		// Push the data structure into the vector
		v.push_back({rectData,flag});
	}
	return true;
}
